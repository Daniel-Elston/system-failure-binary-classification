...
class DatabaseConnection:
    ...
    def log_database_info(self):
        ...
        try:
            ...
            logging.debug("Tables in the database (schema, table):")
            for schema, table in tables:
                table_store.append((schema, table))
            logging.debug(f"{table_store}\n\n")
...
class DataHandler:
    ...
    def fetch_data(self, save_path: Path, query: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            fetched_data = pd.concat(chunks, ignore_index=True)
            FileAccess.save_file(fetched_data, save_path)
